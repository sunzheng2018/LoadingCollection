...
public class SoGouBrowserLoading extends View{
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText("startX:" + startX + " startY:" + startY + " endY:" + endY + " currentY:" + currentY, 10, 40, new Paint(Paint.ANTI_ALIAS_FLAG));
    }
...
//        valueAnimator.setRepeatMode(2);
    ...
    private void playAnimator1(){
        //我们只需要取Y轴方向上的变化即可
        final ValueAnimator valueAnimator = ValueAnimator.ofInt(endY, startY);
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                if(currentY <= startY+11) {
                    valueAnimator.cancel();
                    playAnimator();
                }
                currentY = (Integer) animation.getAnimatedValue();
